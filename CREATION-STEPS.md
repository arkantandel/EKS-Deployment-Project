...
- [🌐 Interactive HTML Guide — Full Content in Markdown](#-interactive-html-guide--full-content-in-markdown)
  - [🎯 Chapter 01 — What is Amazon EKS?](#-chapter-01--what-is-amazon-eks)
  - [🏛️ Architecture — EKS Full Stack Layers](#️-architecture--eks-full-stack-layers)
  - [⚖️ Chapter 02 — EKS vs. Self-Managed Kubernetes](#️-chapter-02--eks-vs-self-managed-kubernetes)
  - [🔄 Workflow — EKS Deployment Pipeline](#-workflow--eks-deployment-pipeline)
  - [🔧 Chapter 03 — AWS Environment Setup](#-chapter-03--aws-environment-setup)
  - [🚀 Chapter 04 — Launching Your EKS Cluster](#-chapter-04--launching-your-eks-cluster)
  - [📦 Chapter 05 — Deploying Applications on EKS](#-chapter-05--deploying-applications-on-eks)
  - [📊 Full Deployment Flow Summary](#-full-deployment-flow-summary)
...
# 🌐 Interactive HTML Guide — Full Content in Markdown

> This section mirrors the complete **AWS EKS Complete Technical Guide** HTML page — every chapter, diagram, code block, and step — presented here in full markdown for GitHub rendering.

---

## 🎯 Chapter 01 — What is Amazon EKS?

> EKS is a fully managed Kubernetes control plane that handles the complexity of cluster operations so your team can focus on building and shipping applications.

### ✨ Core Capabilities

| Feature | Icon | Description |
|---------|------|-------------|
| **Managed Control Plane** | ☸️ | AWS operates the Kubernetes API server, etcd, and controller manager across multiple Availability Zones — you never touch the control plane |
| **Automated Updates** | 🔄 | EKS automatically patches and upgrades the Kubernetes version, ensuring you always run secure, supported releases with zero manual intervention |
| **Native AWS Integration** | 🔗 | Seamless integration with IAM, VPC, Load Balancers, ECR, CloudWatch, and 200+ AWS services — your cluster is a first-class AWS citizen |
| **Auto-Scaling** | 📈 | The control plane scales automatically with demand. Combine with Cluster Autoscaler and Karpenter for fully elastic worker node scaling |
| **Security & Compliance** | 🔒 | SOC 2, PCI DSS, ISO 27001, HIPAA, and FedRAMP certified. IAM-native authentication via aws-iam-authenticator bridges AWS and Kubernetes RBAC |
| **Observability Built-in** | 👁️ | CloudWatch Container Insights provides automatic collection of metrics, logs, and traces from your containerized applications and microservices |

---

## 🏛️ Architecture — EKS Full Stack Layers

```mermaid
graph TB
    subgraph L1["🌐 INTERNET LAYER"]
        I1["🌐 Internet Traffic"]
        I2["👤 Developer kubectl"]
        I3["🔄 CI/CD Pipeline"]
    end

    subgraph L2["⚡ EDGE LAYER"]
        E1["⚖️ Application Load Balancer"]
        E2["🛡️ AWS WAF & Shield"]
        E3["🔐 Route53 DNS"]
    end

    subgraph L3["☸️ EKS CONTROL PLANE — Managed by AWS"]
        C1["🖥️ API Server\nkube-apiserver"]
        C2["📦 etcd\nState Store"]
    ...
    subgraph L4["🖧 DATA PLANE — Your Worker Nodes"]
        N1["🖧 Worker Node 1\nkubelet + kube-proxy"]
        N2["🖧 Worker Node 2\nkubelet + kube-proxy"]
        N3["🖧 Worker Node 3\nkubelet + kube-proxy"]
    end

    subgraph L5["🔧 AWS SUPPORTING SERVICES"]
        S1["🔑 IAM Auth"]
        S2["🌐 VPC Network"]
        S3["📦 ECR Registry"]
        S4["📊 CloudWatch Logs"]
    ...
    L1 -->|traffic| L2
    L2 -->|requests| L3
    L3 -->|schedules pods| L4
    L4 <-->|integrates| L5

    style L3 fill:#0D2137,stroke:#00C6FF,stroke-width:3px,color:#fff
    style L4 fill:#0A1E2E,stroke:#10B981,stroke-width:2px,color:#fff
    style L5 fill:#1A1A0D,stroke:#F59E0B,stroke-width:2px,color:#fff
```

---

## ⚖️ Chapter 02 — EKS vs. Self-Managed Kubernetes

> Choose the right deployment model for your team's expertise, budget, and operational requirements.

### Amazon EKS — Fully Managed Service ☸️

| Type | Feature |
|------|---------|
| ✅ **PRO** | Managed control plane — zero ops overhead |
| ✅ **PRO** | Automated Kubernetes version upgrades |
| ✅ **PRO** | Auto-scaling control plane on demand |
| ✅ **PRO** | Native IAM, VPC, ELB, ECR integration |
| ✅ **PRO** | SOC / PCI / HIPAA compliance out of the box |
| ✅ **PRO** | CloudWatch monitoring built-in |
| ❌ **CON** | Higher monthly cost vs self-managed |
| ❌ **CON** | Less control over control plane configuration |

### Self-Managed Kubernetes — DIY on EC2 ⚙️

| Type | Feature |
|------|---------|
| ✅ **PRO** | Cost savings with Spot + Reserved Instances |
| ✅ **PRO** | Full control over all Kubernetes configurations |
| ✅ **PRO** | Access to experimental / alpha Kubernetes features |
| ✅ **PRO** | AWS service integration still available |
| ❌ **CON** | Complex setup & ongoing maintenance burden |
| ❌ **CON** | Manual control plane HA and updates |
| ❌ **CON** | Scaling the control plane is challenging |
| ❌ **CON** | Security configuration requires extra effort |

> 💡 **Recommendation:** For teams prioritizing velocity and reliability, EKS is the clear choice. Reserve self-managed Kubernetes for organizations with dedicated platform engineering teams who need extreme customization or are optimizing costs at massive scale (1000+ nodes).

---

## 🔄 Workflow — EKS Deployment Pipeline

> From code commit to running container — the complete end-to-end workflow for deploying applications on EKS.

```mermaid
flowchart LR
    S1["💻\n**Step 1**\nCode Commit\nGit push triggers pipeline"]
    S2["🐳\n**Step 2**\nDocker Build\nMulti-stage Dockerfile"]
    S3["📦\n**Step 3**\nPush to ECR\nTagged image registry"]
    S4["📄\n**Step 4**\nYAML Manifest\nDeployment + Service spec"]
    S5["⚙️\n**Step 5**\nkubectl apply\nRolling deployment"]
    S6["✅\n**Step 6**\nLive & Healthy\nReadiness probe passes"]

    S1 --> S2 --> S3 --> S4 --> S5 --> S6

    style S1 fill:#0D2137,stroke:#00C6FF
    style S2 fill:#0D2137,stroke:#2496ED
    style S3 fill:#0D2137,stroke:#FF9900
    style S4 fill:#0D2137,stroke:#F59E0B
    style S5 fill:#0D2137,stroke:#8B5CF6
    style S6 fill:#0A2E1A,stroke:#10B981
```

---

## 🔧 Chapter 03 — AWS Environment Setup

> Configure identity, networking, and permissions before launching any cluster resources.

---

### 📌 Step 01 — Create AWS Account & Enable MFA

Register at [aws.amazon.com](https://aws.amazon.com) and immediately enable Multi-Factor Authentication on the root account. Never use root credentials for daily operations.

> ⚠️ **Critical:** Create a dedicated IAM admin user and lock away your root credentials after initial setup.

---

### 📌 Step 02 — Create IAM Users & Roles

Set up IAM users with least-privilege permissions. Create an EKS cluster role and a worker node instance role with the required managed policies attached.

```bash
# EKS Cluster Role requires:
AmazonEKSClusterPolicy
AmazonEKSServicePolicy

# Worker Node Role requires:
...
### 📌 Step 03 — Configure AWS CLI & kubectl

Install both tools and authenticate. The AWS CLI handles AWS API calls while kubectl communicates directly with the Kubernetes API server.
...
### 📌 Step 04 — Set Up VPC & Networking

Create a dedicated VPC with public subnets (for load balancers) and private subnets (for worker nodes), distributed across at least 2 Availability Zones.

```mermaid
graph TB
    subgraph VPC["🌐 Amazon VPC — 10.0.0.0/16"]
        subgraph PUBLIC["🌍 Public Subnets — Load Balancers"]
            PUB_A["Subnet 10.0.1.0/24\nus-east-1a"]
            PUB_B["Subnet 10.0.2.0/24\nus-east-1b"]
        end
        subgraph PRIVATE["🔒 Private Subnets — Worker Nodes"]
            PRIV_A["Subnet 10.0.10.0/24\nus-east-1a"]
            PRIV_B["Subnet 10.0.20.0/24\nus-east-1b"]
            PRIV_C["Subnet 10.0.30.0/24\nus-east-1c"]
        end
    ...
    style PUBLIC fill:#1A1A0D,stroke:#F59E0B
    style PRIVATE fill:#0A1E2E,stroke:#10B981
    style VPC fill:#080F1A,stroke:#2E75B6,stroke-width:2px
```

> ℹ️ **Subnet Tagging:** EKS requires specific tags on subnets.
> - Public subnets: `kubernetes.io/role/elb = 1`
> - Private subnets: `kubernetes.io/role/internal-elb = 1`

---

## 🚀 Chapter 04 — Launching Your EKS Cluster

> Create the cluster using the AWS CLI for repeatable, scriptable infrastructure provisioning.

---

### 📌 Step 05 — Create the EKS Cluster

Use the AWS CLI to create your cluster. This provisions the managed control plane — typically takes 10–15 minutes to become active.
...
### 📌 Step 06 — Add Worker Node Group

Create a managed node group to provide compute capacity. EKS will automatically register nodes and join them to the cluster.
...
### 📌 Step 07 — Authenticate & Verify Cluster

Update your local kubeconfig to point kubectl at the new cluster and verify all nodes are in Ready state before deploying any workloads.
...
## 📦 Chapter 05 — Deploying Applications on EKS

> Package, configure, and deploy containerized applications using Docker and Kubernetes manifests.

---

### 📌 Step 08 — Containerize with Docker

Build a production-optimized Docker image using multi-stage builds to minimize final image size, then push it to Amazon ECR.
...
### 📌 Step 09 — Write Kubernetes Deployment YAML

Define your application's desired state using a Deployment (pod management) and Service (network exposure) manifest.
...
### 📌 Step 10 — Deploy & Verify Application

Apply the manifests and monitor the rolling deployment until all pods reach the Running state and the LoadBalancer endpoint becomes available.
...
# Stream live logs
kubectl logs -l app=my-application --tail=50 -f

# Scale to 5 replicas
kubectl scale deployment/my-application --replicas=5
```

> ✅ **Success indicator:** When `kubectl get pods` shows all pods with `STATUS=Running` and `READY=1/1`, your application is **live and serving traffic.** 🎉
...
sequenceDiagram
    ...
    DOCKER->>ECR: docker push :latest
    ECR-->>DOCKER: ✅ image stored

    DEV->>EKS: kubectl apply -f deployment.yaml
    EKS->>ECR: pull image
    ECR-->>EKS: image downloaded
    EKS->>EKS: schedule pods on nodes
    EKS->>ALB: register targets

    ALB-->>DEV: EXTERNAL-IP assigned
    DEV->>ALB: curl http://EXTERNAL-IP
    ALB-->>DEV: 200 OK 🎉
```
...
*End of Interactive HTML Guide Content*
